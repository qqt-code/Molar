...
class Data:
    def __init__(self, config):
        self.config = config
        self.dataset_path = config['data_path'] # ../dataset/
        self.dataset_name = config['dataset'] #Pixel50k
        self.data_split = config['data_split'] #None
        self.item_data = config['item_data'] #None
    ...
    def _data_processing(self):
        ...
        self.user_num = len(self.id2token['user_id']) # 50001
        self.item_num = len(self.id2token['item_id']) # 82865
        ...
        self.inter_num = len(self.inter_feat) # 989476对的1 user - 1 item
    ...
    def build(self):
        ...
        self.sort(by='timestamp') #根据时间戳排序
        ...
        grouped_index = self._grouped_index(user_list) #建立user 与 item list的对应关系
        ...
        for index in grouped_index.values():
            indices.extend(list(index)[:-2])#建立训练集，最后2个元素不取
        ...
        elif self.config['MODEL_INPUT_TYPE'] == InputType.SEQ: #进这个
    ...
    def _build_seq(self, train_feat):
        max_item_list_len = self.config['MAX_ITEM_LIST_LENGTH']+1 # 10 + 1
