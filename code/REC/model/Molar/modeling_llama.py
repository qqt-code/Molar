...
def _make_causal_mask(
    ...
    """
    bsz, tgt_len = input_ids_shape # 批量大小和目标长度
...
class LlamaLinearScalingRotaryEmbedding(LlamaRotaryEmbedding):
    ...
    def _set_cos_sin_cache(self, seq_len, device): #预先计算cos和sin值，以加速前向计算
        self.max_seq_len_cached = seq_len
        t = torch.arange(self.max_seq_len_cached, device=device, dtype=torch.float32)
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        past_key_value = (key_states, value_states) if use_cache else None #use_cache = false
        ...
        if not self.use_ft_flash_attn:
            ...
            attn_output = compute_flash_attention(
                ...
                True,
                self.training,
            )#self.training = true attention_mask = None
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        """
        #hidden_states.shape = (1,seq_len,2048)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if past_key_values is not None: #不进
            past_key_values_length = past_key_values[0][0].shape[2]
            seq_length_with_past = seq_length_with_past + past_key_values_length

        if position_ids is None: #不进
        ...
        if inputs_embeds is None: #不进
            inputs_embeds = self.embed_tokens(input_ids)
        # embed positions
        if cu_input_lens is None: #不进
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training: #进这个
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        ```"""
        # inputs_embeds.shape = (1,14606,2048) cu_input_lens.shape = (176) position_ids.shape = (1,14606),其他均为none
        output_attentions = (
            output_attentions
            if output_attentions is not None
            else self.config.output_attentions
